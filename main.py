...
def style_table_like_sample(ws, top_row:int, left_col:int, rows:int, cols:int, header_rows:int=1, header_fill="EFC9B8"):
    """Arabic RTL table styling similar to the screenshots."""
    ...
    for r in range(top_row, top_row + rows):
        ws.cell(row=r, column=left_col).border = Border(
            top=ws.cell(row=r, column=left_col).border.top, left=thick,
...
def add_issue(data: Issue):
    with Session(engine) as s:
        s.add(data); s.commit(); s.refresh(data)
        return data

@app.get("/api/issue")
def list_issue():
    with Session(engine) as s:
        return s.exec(select(Issue).order_by(Issue.issue_date.desc(), Issue.id.desc())).all()
...
def export_issue_full():
    ...
    wb = Workbook(); ws = wb.active; ws.title = "الصرف"
    ws.sheet_view.rightToLeft = True
    ...
    for row in ws.iter_rows(min_row=1, max_row=ws.max_row, min_col=1, max_col=len(headers)):
        for c in row: c.border = Border(top=thin, left=thin, right=thin, bottom=thin)
...
def export_issue_summary():
    # name, qty, serial, current location, receiver
    with Session(engine) as s:
        rows = s.exec(select(Issue).order_by(Issue.issue_date, Issue.id)).all()
...
def add_cabinet(data: CabinetRehab):
    # enforce unique code if provided
    if data.code:
        with Session(engine) as s:
            exists = s.exec(select(CabinetRehab).where(CabinetRehab.code == data.code)).first()
            if exists: raise HTTPException(400, "الترميز موجود مسبقًا")
    with Session(engine) as s:
        s.add(data); s.commit(); s.refresh(data); return data

@app.get("/api/cabinets")
def list_cabinets():
    with Session(engine) as s:
        return s.exec(select(CabinetRehab).order_by(CabinetRehab.rehab_date.desc(), CabinetRehab.id.desc())).all()
...
def export_cabinets(year: int, month: int):
    with Session(engine) as s:
        rows = [r for r in s.exec(select(CabinetRehab)).all() if _month_filter(r.rehab_date, year, month)]
...
def add_asset(data: AssetRehab):
    # unique serial if provided for same asset
    if data.serial_or_code:
        with Session(engine) as s:
            exists = s.exec(select(AssetRehab).where(AssetRehab.serial_or_code == data.serial_or_code)).first()
            if exists: raise HTTPException(400, "الرقم التسلسلي/الترميز موجود مسبقًا")
    with Session(engine) as s:
        s.add(data); s.commit(); s.refresh(data); return data
...
def list_assets():
    with Session(engine) as s:
        return s.exec(select(AssetRehab).order_by(AssetRehab.supply_date.desc(), AssetRehab.id.desc())).all()
...
def update_asset(aid: int, data: AssetRehab):
    with Session(engine) as s:
        ...
        if data.serial_or_code and data.serial_or_code != obj.serial_or_code:
            ...
            if dup: raise HTTPException(400, "الرقم التسلسلي/الترميز موجود مسبقًا")
        for k, v in data.dict().items():
            if k == "id": continue
            setattr(obj, k, v)
        s.add(obj); s.commit(); s.refresh(obj); return obj
...
def stats_assets(year: int, month: int):
    ...
    with Session(engine) as s:
        for r in s.exec(select(AssetRehab)).all():
            if _month_filter(r.supply_date, year, month) and r.asset_type in res:
                res[r.asset_type] += (r.quantity or 1)
    return res

@app.get("/api/export/assets.xlsx")
def export_assets(year: int, month: int):
    with Session(engine) as s:
        rows = [r for r in s.exec(select(AssetRehab)).all() if _month_filter(r.supply_date, year, month)]
    ...
    for r in rows:
        ws.append([r.asset_type, r.model, r.serial_or_code, r.quantity, r.prev_location, r.supply_date,
                   ...
                   r.requester, r.receiver, r.notes])
    thin = Side(border_style="thin", color="999999")
    for row in ws.iter_rows(min_row=1, max_row=ws.max_row, min_col=1, max_col=len(headers)):
        for c in row: c.border = Border(top=thin, left=thin, right=thin, bottom=thin)
    buf = io.BytesIO(); wb.save(buf); buf.seek(0)
...
def list_spares():
    with Session(engine) as s:
        return s.exec(select(SparePartRehab).order_by(SparePartRehab.rehab_date.desc(), SparePartRehab.id.desc())).all()
...
def validate_duplicates():
    with Session(engine) as s:
        ...
        codes = {}
        for r in s.exec(select(CabinetRehab)).all():
            if r.code: codes[r.code] = codes.get(r.code, 0) + 1
...
# ---------------- Monthly Summary export (styled like screenshots) ----------------
...
def export_monthly_summary(year: int, month: int):
    ...
    for i, (label, (kind, key)) in enumerate(rows_map, start=1):
        ...
        if kind == "cab": val = cab_cnt.get(key, 0)
        elif kind == "ast": val = ast_cnt.get(key, 0)
        else: val = spa_cnt.get(key, 0)
...
def export_quarterly_summary(start_year: int, start_month: int):
    ...
    for i,(label,_) in enumerate(rows_map, start=1):
        ...
        for mi in range(3):
            ...
            row_sum += v; totals_per_month[mi] += v
