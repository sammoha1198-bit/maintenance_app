...
class AssetRehab(SQLModel, table=True):
    ...
    receiver: Optional[str] = None
    notes: Optional[str] = None
    # للأصول: التقارير/الرسوم تعتمد على rehab_date (مع سقوط إلى supply_date عند غيابه)
...
# ========= Self-migration: add AssetRehab.rehab_date if missing =========
...
def _ensure_assetrehab_rehab_date():
    # create tables if needed
    SQLModel.metadata.create_all(engine)

    insp = inspect(engine)
    if "assetrehab" not in insp.get_table_names():
        return
    if not _has_column("assetrehab", "rehab_date"):
        with engine.begin() as conn:
            conn.exec_driver_sql("ALTER TABLE assetrehab ADD COLUMN rehab_date DATE;")
            try:
...
def stats_cabinets(year: int, month: int):
    ...
    with Session(engine) as s:
        q = select(CabinetRehab).where(year_eq(CabinetRehab.rehab_date, year), month_eq(CabinetRehab.rehab_date, month))
        for r in s.exec(q).all():
            if r.cabinet_type in res:
...
async def add_asset(req: Request):
    ...
    with Session(engine) as s:
        if item.serial_or_code and _asset_duplicate_exists(s, item.serial_or_code):
            raise HTTPException(status_code=400, detail="هناك تكرار في الرقم التسلسلي/الترميز")
        s.add(item)
        s.commit()
        s.refresh(item)
        return item
...
def stats_assets(
    ...
    # اختيار العمود: rehab_date افتراضيًا؛ ويمكن طلب supply_date
    col = func.coalesce(AssetRehab.rehab_date, AssetRehab.supply_date) if date_field != "supply_date" else AssetRehab.supply_date
...
def validate_duplicates():
    with Session(engine) as s:
        ...
        # Assets: duplicate serials and (serial, current_location)
...
def export_monthly_summary(year: int, month: int):
    ...
    with Session(engine) as s:
        ...
        # أصول (المطلوب تجميعها وفق تاريخ التأهيل؛ إن لم يوجد فالتوريد) — نستخدم coalesce للتصفية لكن نجمع بالعدد
...
def export_quarterly_summary(start_year: int, start_month: int):
    ...
    with Session(engine) as s:
        for (yy, mm) in months:
            ...
            # Assets (sum quantities, filter by rehab_date fallback)
            col = func.coalesce(AssetRehab.rehab_date, AssetRehab.supply_date)
            ast_rows = s.exec(
                select(AssetRehab.asset_type, func.coalesce(func.sum(AssetRehab.quantity), 0))
                .where(col.is_not(None), year_eq(col, yy), month_eq(col, mm))
                .group_by(AssetRehab.asset_type)
            ).all()
            ast = {"بطاريات": 0, "موحدات": 0, "محركات": 0, "مولدات": 0, "مكيفات": 0, "أصول أخرى": 0}
            for k, v in ast_rows:
    ...
    for i, (label, _) in enumerate(rows_map, start=1):
        ...
        for mi in range(3):
            ...
            row_sum += v
            total_per_month[mi] += v
        ws.cell(row=r, column=6, value=row_sum)
        grand_total += row_sum
        r += 1
    ws.cell(row=r, column=2, value="الإجمالي").font = Font(bold=True)
    for mi in range(3):
        ws.cell(row=r, column=3 + mi, value=total_per_month[mi]).font = Font(bold=True)
